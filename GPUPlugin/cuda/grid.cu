...
__global__ void init(const GPUPseudoJet *particles, PseudoJetExt *jets, int size) {
  ...
  for (int i = first; i < size; i += stride) {
    ...
    jets[i].index = i; //particles[i]._user_index;
    jets[i].isJet = false; // particles[i].isJet;
  }
}

__global__ void output(const PseudoJetExt *jets, GPUPseudoJet *particles, int size) {
  int first = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
  ...
  for (int i = first; i < size; i += stride) {
    ...
    // particles[i].rap = jets[i].rap;
    // particles[i].phi = jets[i].phi;
    // particles[i].diB = jets[i].px * jets[i].px + jets[i].py * jets[i].py;
    // particles[i]._user_index = jets[i].isJet;
    particles[i].isJet = jets[i].isJet;
  }
}

void GPUCluster::cluster(GPUPseudoJet *particles, int size, Algorithm algo, double r) {
